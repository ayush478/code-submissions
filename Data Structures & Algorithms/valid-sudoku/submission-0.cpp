class Solution {
   public:
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        int blocks[3][3][9] = {0};   // blocks[0][2][5] means whether number '6' in block 0,2 (row
                                     // 0~2,col 6~8) has appeared.
        for (int r = 0; r < 9; r++)  // traverse board r,c
            for (int c = 0; c < 9; c++)
                if (board[r][c] != '.') {  // skip all number '.'
                    int number =
                        board[r][c] - '1';  // calculate the number's index(board's number minus 1)
                    if (rows[r][number]++)
